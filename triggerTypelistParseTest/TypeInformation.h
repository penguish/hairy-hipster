...
struct list {
  ...
  template <class new_element>    struct v_addWithChecking;
...
template<class element,
    class list,
    template<class E,class L,int I> class by,
    int index = list::last_index,
    int fall_back =-1>
struct find{
...
struct newlist{
  ...
  template <class first_element>struct v_addWithChecking{typedef list<first_element,nil> go;};
};

//type_info case

template<class new_element,class thelist, int index>
struct do_my_add;
...
struct list<type,list_of_types>::addWithChecking<type_info<O,F,C,A> >{
  typedef type_info<O,F,C,A> edmType;
  typedef list<type,list_of_types> thisList;
  static const int thisIndex = find<typename edmType::container,thisList,get_cont>::result;
  typedef typename do_my_add<edmType,list<type,list_of_types>,thisIndex>::result go;
};
    
    template <class type, class list_of_types>
    template<class O,class F,typename...contAux>
    struct list<type,list_of_types>::v_addWithChecking<type_info<O,F,contAux...> >{
        typedef typename do_my_add<type_info<O,F,contAux...>,list<type,list_of_types>,find<typename type_info<O,F,contAux...>::container,list<type,list_of_types>,get_cont>::result>::result go;
    };
...
//   result: the result struct to be used
...
struct for_each_type{
  static void do_it(const functor* f = 0) {
    //transient object to use in case no object given, must have default ctor
